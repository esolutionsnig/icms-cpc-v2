jQuery(document).ready(function($) {
    ...
    $("#erName").keyup(function() {
        ...
        if (erName == '') {
            Materialize.toast('Preannouncement Title Is Required', 3000, 'rounded')
        }
    })

    // Auto fetch client branch 
    $(".cname").change(function(){
        var cid = $(this).val()
        $.ajax({
            url: 'getClientBranches.php',
            type: 'post',
            data: {cid:cid},
            success:function(response){
                $(".cbranch").html(response)
                console.log(response)
            }
        })
    })

    // Auto Fill Branch Location and Code
    ...
    $(".proceedToCashAllocation").click(function(event) {
        ...
        if (erName == '' || clientBranch == '' || clientBranchL == '' || clientBranchLC == '' || consignmentLocation == '') {
            ...
            $.ajax({
                url: "app/pf/preannouncements.php",
                method: "POST",
                data: { initiateP: 1, erName: erName, erSlug: erSlug, bankId:bankId, consignmentLocation:consignmentLocation, clientBranchLC: clientBranchLC, clientBranch:clientBranch, clientBranchL: clientBranchL, username: username, usertoken: usertoken },
                success: function(data) {
                    $(".proceedToCashAllocation").html('<i class="material-icons right">save</i> Save Preannouncement ')
                    $(".proceedToCashAllocation").removeAttr('disabled')
                    if (data == "rsent") {
                        Materialize.toast('Preannouncement Successfully Initiated', 1500, 'rounded')
                        window.setTimeout(function() {
                            location.href='preannouncements'
                        }, 1500)
                    } else {
                        console.log(data)
    ...
    $(".consignmentHandOver").click(function(event) {
        ...
        if (username == '' || usertoken == '' || erId == '' || tap == '' || bbCid == '' || citConfirmationToken == '') {
            Materialize.toast('Request Denied: Kindly Fill All Fields To Proceed', 2000, 'rounded');
        } else {
            ...
            $.ajax({
                url: "app/pf/preannouncements.php",
                method: "POST",
                data: { addCHO: 1, citConfirmationToken: citConfirmationToken, cmo: cmo, tap: tap, bbCid: bbCid, vehicle: vehicle, erId: erId,  username: username, usertoken: usertoken },
                success: function(data) {
                    ...
                    if (data == "choAdded") {
                        Materialize.toast('Consignment Confirmation Successfully', 1000, 'rounded')
                        window.setTimeout(function() {
                            location.href='preannouncements'
                        }, 1000)
                    } else {
