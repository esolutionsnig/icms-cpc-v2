...
include 'app/pf/cash-requests.php';
include 'app/core/session.php';
$pagename = 'Cash Requests';
...
<html lang="en">
  ...
  <body>
          ...
          <div class="container">
            <div class="section">
              <div class="row">
                <div class="col s12">
                  <div class="right">
                    <a href="cash-request-c" class="btns btns-add waves-effect waves-teal" style="color: white !important"><i class="material-icons left">add</i> Make New Request</a>
                  </div>
                </div>
              </div>
              <div class="row">
                <div class="col s12">
                  <div class="card">
                    <div class="card-content">
                      ...
                      <?php
                        $sno = 1;
                        $sql = "SELECT * FROM bank_requests WHERE preannounced = 'NO' AND client_rep = '$username' ORDER BY er_id DESC";
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<tr>
                                    <td>' . $sno++ . '</td>
                                    <td><a href="cash-request-r?r=' . $row['er_slug'].'cprf'.$bankUID .'" style="color: #504c75 !important;">' . $row['er_name'] . '</a></td>
                                    <td>' . $clientName . '</td>
                                    ...
                                    <td>
                                        <a href="cash-request-r?r=' . $row['er_slug'].'cprf'.$bankUID .'" class="btns btns-read waves-effect waves-light" style="color: teal !important; margin: 3px !important;">
                                            View  <i class="material-icons left">link</i>
                                        </a>
                                        ';
                                        if ($row['cp_done'] == 'NO') {
                                            echo '<a href="cash-request-p?r=' . $row['er_slug'].'cprf'.$bankUID .'" class="btns btns-edit waves-effect waves-light" style="color: white !important; margin: 3px !important;">
                                            Prepare Cash <i class="material-icons left">local_mall</i>
                                            </a>';
                                        }
                                        if ($session->isSuperAdmin() || $session->isBankerCmu()) {
                                          if ( $row['cp_done'] == 'NO' ) {
                                              echo '<a href="cash-request-e?r=' . $row['er_slug'].'cprf'.$bankUID .'" class="btns light-blue waves-effect waves-light" style="color: white !important; margin: 3px !important;">
                            ...
                            echo 'No request found. <a href="cash-request-c" class="btns btns-add waves-effect waves-teal" style="color: white !important"><i class="material-icons left">add</i> Make New Request</a>';
                        }
                      ?>
    ...
    <script type="text/javascript" src="assets/js/pages/cash-requests.js"></script>
