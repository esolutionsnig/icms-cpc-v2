...
if(isset($_POST["addNewNotification"])){

	$notTitle	 	= mysqli_real_escape_string($con, $_POST['notName']);
	$notBody	 	= mysqli_real_escape_string($con, $_POST['notBody']);
	$created_at 	= time();
	...
	if( !$error ) {
		...
		if($counta > 0){
			while($rowa = mysqli_fetch_array($run_querya)){
				...
				$sql = "INSERT INTO notifications (username, subject, message, status, recOn) 
				VALUES ('$recName', '$notTitle', '$notBody', 'Unread', '$created_at')";
